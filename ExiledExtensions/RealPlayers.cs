...
namespace Gamer.Utilities
{
    public static class RealPlayers
    {
        ...
        public static Player Get(string value) => List.Where(p => p.UserId == value || value.Split('.').Contains(p.Id.ToString()) || p.Nickname == value || p.DisplayNickname == value).FirstOrDefault();
