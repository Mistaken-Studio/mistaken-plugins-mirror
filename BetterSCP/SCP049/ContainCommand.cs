...
namespace Gamer.Mistaken.BetterSCP.SCP049.Commands
{
    ...
    public class ContainCommand : IBetterCommand
    {
        ...
        {
            scp049.ShowHint("<color=red><size=150%>You are being recontained</size></color><br>Stand still for <color=yellow>5</color>s", true, 2, true);
            ...
            if (RealPlayers.List.Any(p => p.Role != RoleType.Scp079 && p.Team == Team.SCP) && RealPlayers.Any(RoleType.Scp079))
                Mistaken.Systems.Patches.SCP079RecontainPatch.Prefix(true);
            else
